...
vector<double> Processor::GetCpuVector() {
    ...
    if (filestream.is_open()) {
      ...
      while(linestream >> tmp) {
          try { cpuValues.push_back(std::stod(tmp)); } 
...
float Processor::Utilization() { 
    ...
    if (!Processor::prevVec.empty()) {
        ...
        double prevNonIdle = Processor::prevVec[0] + Processor::prevVec[1] + Processor::prevVec[2] + Processor::prevVec[5] + Processor::prevVec[6] + Processor::prevVec[7];
        double nonIdle = Processor::currVec[0] + Processor::currVec[1] + Processor::currVec[2] + Processor::currVec[5] + Processor::currVec[6] + Processor::currVec[7];
